        ...
        <div className="flex flex-col items-center gap-4">
          {navData.map((item, index) => (
            <Link
              key={index}
              to={item.url}
              className={`block px-6 py-3 font-light text-black transition-colors duration-300 ease-in-out 
                ${active === index + 1
      ...
      <div className="hidden lg:flex w-full justify-center gap-6">
        {navData.map((item, index) => (
          <Link
            key={index}
            to={item.url}
            className={`px-2 py-2 font-light capitalize text-blacktransition-colors duration-300 ease-in-out 
              ${active === index + 1
                ? 'text-blue-900 underline underline-offset-4'
                : 'hover:text-blue-900 hover:underline hover:underline-offset-4'}`}
          >
